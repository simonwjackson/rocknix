...
grep -q 'boot.isContainer = true' "${PKG_DIR}/guest/rocknix-guest.nix" || fail "Layer 10b guest must be a container-style rootfs"
grep -q 'services.openssh = {' "${PKG_DIR}/guest/rocknix-guest.nix" || fail "Layer 12 guest must define locked-down OpenSSH"
grep -q 'authorizedKeysFiles = \[ "/etc/ssh/authorized_keys.d/%u" \]' "${PKG_DIR}/guest/rocknix-guest.nix" || fail "Layer 12 guest must use runtime-mounted authorized keys"
grep -q 'PasswordAuthentication = false' "${PKG_DIR}/guest/rocknix-guest.nix" || fail "Layer 12 guest must disable SSH password authentication"
grep -q 'KbdInteractiveAuthentication = false' "${PKG_DIR}/guest/rocknix-guest.nix" || fail "Layer 12 guest must disable keyboard-interactive authentication"
grep -q 'PermitRootLogin = "prohibit-password"' "${PKG_DIR}/guest/rocknix-guest.nix" || fail "Layer 12 guest root SSH must be key-only"
grep -q 'users.users.root.hashedPassword = "!"' "${PKG_DIR}/guest/rocknix-guest.nix" || fail "Layer 10b guest must lock root password"
! grep -q 'ssh-rsa\|ssh-ed25519\|ecdsa-sha2-' "${PKG_DIR}/guest/rocknix-guest.nix" || fail "Layer 12 guest must not ship authorized keys"
! grep -q '/dev/dri\|PipeWire\|PulseAudio\|/dev/input\|Steam\|FEX\|WAYLAND_DISPLAY' "${PKG_DIR}/guest/rocknix-guest.nix" || fail "Layer 10b guest must not reference forbidden passthrough surfaces"
