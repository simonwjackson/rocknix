...
grep -q 'aarch64-linux' "${PKG_DIR}/guest/flake.nix" || fail "Layer 10b guest flake must target aarch64-linux"
...
grep -q 'services.openssh.enable = false' "${PKG_DIR}/guest/rocknix-guest.nix" || fail "Layer 10b guest must not enable SSH"
grep -q 'users.users.root.hashedPassword = "!"' "${PKG_DIR}/guest/rocknix-guest.nix" || fail "Layer 10b guest must lock root password"
! grep -q 'services.openssh.enable = true' "${PKG_DIR}/guest/rocknix-guest.nix" || fail "Layer 10b guest must not enable SSH"
