...
GUEST_CEMU="${GUEST_CEMU:-/nix/store/wl4g8jjlw6pck4sh4ayah9pdl03z8brp-cemu-2.999.0/bin/Cemu}"
...
guest_file_section() {
  ...
  append_guest_cmd "$label runtime data" "out=\$(dirname \$(dirname '$bin')); for f in \"\$out/share/Cemu/gameProfiles/default/00050000101c9400.ini\" \"\$out/share/Cemu/resources/sharedFonts/CafeCn.ttf\" \"\$out/share/Cemu/config/SM8550/settings.xml\"; do if [ -f \"\$f\" ]; then ls -l \"\$f\"; else echo missing: \"\$f\"; fi; done"
...
printf -- '- ROCKNIX-style candidate: `projects/ROCKNIX/packages/tools/nix-integration/guest/flakes/cemu/rocknix-style.nix`\n' >> "$REPORT"
printf -- '- Faithful candidate: `projects/ROCKNIX/packages/tools/nix-integration/guest/flakes/cemu/rocknix-faithful.nix`\n' >> "$REPORT"
...
printf -- '- Direct package candidate: `projects/ROCKNIX/packages/tools/nix-integration/guest/flakes/cemu/rocknix-package.nix`\n' >> "$REPORT"
